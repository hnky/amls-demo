...
parser.add_argument("--data-folder", type=str, dest="data_folder", help="data folder mounting point", default="C:\\Demos\\SimpsonsDemo\\")
parser.add_argument("--pic-size", type=int, dest="pic_size", help="", default=128)
parser.add_argument("--batch-size", type=int, dest="batch_size", help="", default=32)
parser.add_argument("--epochs", type=int, dest="epochs", help="data folder mounting point", default=100)
...
### Add a callback for accuracy and loss graph
class AccuracyGraph(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        run.log("Accuracy", np.float(logs.get("acc")))
...
def load_pictures(BGR):
    ...
    for k, char in map_characters.items():
        pictures = [k for k in glob.glob(os.path.join(data_path,"characters/%s/*" % char))]
...
def training(model, X_train, X_test, y_train, y_test, data_augmentation=True):
    ...
    if data_augmentation:
        ...
        callbacks_list = [LearningRateScheduler(lr_schedule) ,checkpoint,accuracy_graph]
